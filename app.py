    ...
    for proj in projects:
        ...
        for idx, ln in enumerate(lines):
            ...
            if 1 < len(ln.split()) <= 6 and ln[0].isupper() and ln.isalpha() == False:
                # skip if contains many punctuation; rely on previous rules
                pass
        ...
        if heading_indices and len(heading_indices) > 0:
            ...
            for i in range(len(heading_indices)-1):
                ...
                if not pdesc:
                    # if no following lines, try include the heading's remainder
                    pdesc = ''
                # detect tech
            ...
            # fallback: try to split by blank-line groups or markers like 'Customer Banking Campaign Analysis:' inside raw desc
            raw = desc
            # common subproject separators
            separators = ['\n\n', '\n- ', '\n• ', '\n\n\n']
            split_points = None
...
def extract_additional_info(text: str) -> Dict[str, Any]:
    ...
    for line in lines:
        ...
        if low.startswith('certificat') or low.startswith('certification'):
            ...
            continue
        # Only treat as languages section when heading explicitly refers to languages
            ...
            if connection:
                with connection.cursor() as cursor:
                    sql = """
                        ...
                        (name, email, resume_score, timestamp, page_no, user_level, actual_skills, education, experience)
...
async def get_profiles(limit: int = 10):
    ...
    try:
        connection = get_db_connection()
        if not connection:
            return {"profiles": [], "error": "Database not available"}
        
        with connection.cursor() as cursor:
            cursor.execute("SELECT * FROM user_data ORDER BY timestamp DESC LIMIT %s", (limit,))
